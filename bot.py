...
def style_data(style_key: str) -> tuple[str, discord.Color, str, str]:
    return STYLE_OPTIONS.get(style_key, STYLE_OPTIONS["aurora"])
...
def setup_embed(view: PartnershipSetupView | None = None) -> discord.Embed:
    ...
    style_name = style_data(view.visual_style)[0] if view else "Aurora"

    embed = discord.Embed(
        title="✦ Pannello Partnership",
        description=(
            "⟡ Seleziona il manager della partnership, scegli lo stile visuale "
            "e poi apri il modulo per inserire il server partner."
        ),
        color=COLOR_PANEL,
    )
    embed.add_field(name="❖ Manager", value=manager, inline=True)
    embed.add_field(name="✧ Stile", value=f"`{style_name}`", inline=True)
    embed.add_field(
...
def intro_saved_embed(draft: PartnershipDraft) -> discord.Embed:
    style_name, color, symbol, style_description = style_data(draft.visual_style)

    embed = discord.Embed(
        title=f"{symbol} Dettagli salvati",
        description=(
            "⟡ Le informazioni principali sono pronte.\n"
            "❖ Ora puoi inserire la descrizione completa del server partner."
        ),
        color=color,
    )
    embed.add_field(name="✦ Server partner", value=draft.partner_intro, inline=False)
    embed.add_field(name="⚡ Manager", value=user_block(draft.manager), inline=True)
    embed.add_field(name="✧ Stile", value=f"`{style_name}`\n{style_description}", inline=True)
...
def partnership_embed(
    *,
    ...
    style_name, color, symbol, _ = style_data(draft.visual_style)

    embed = discord.Embed(
        title=f"{symbol} Nuova Partnership",
        description=(
    ...
    embed.add_field(name="✦ Profilo visuale", value=f"`{style_name}`", inline=True)
...
class StyleSelect(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            ...
            for key, (style_name, _, symbol, description) in STYLE_OPTIONS.items()
        ]
        super().__init__(
            placeholder="Seleziona lo stile dell'embed",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        view.visual_style = self.values[0]
        for option in self.options:
            option.default = option.value == view.visual_style
...
class PartnerIntroModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        draft = PartnershipDraft(
            ...
            visual_style=self.setup_view.visual_style,
...
async def crea_partnership_error(
    ...
    log.exception("Errore nel comando /crea-partnership", exc_info=root_error)
    await safe_ephemeral_response(
        interaction,
        embed=error_embed("❖ Si e verificato un errore durante l'apertura del pannello."),
    )
